    ...
    return (
        <div className={`sidebar-container ${isOpen ? 'open' : ''}`}>
            ...
            <nav className="sidebar-nav">
                ...
                {/* Add Member */}
                {(isAdmin || roles.includes("ADD_MEMBER")) && (
                <Link to="/add-member" className={`sidebar-nav-item ${location.pathname === '/add-member' ? 'active' : ''}`} onClick={handleNavigation}>
                    <Icon path={<><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><path d="M21 8v6"/><path d="M24 11h-6"/><circle cx="8" cy="7" r="4"/></>} />
                ...
                {(isAdmin || roles.includes("ACCOUNTING")) && (
                    <>
                        <div className="sidebar-menu-category" style={{marginTop: '16px'}}>FINANCE</div>
                        <div className={`sidebar-collapsible ${openMenus.accounting ? 'is-open' : ''}`}>
                            ...
                            <div className="sidebar-sub-menu">
                                ...
                                <Link to="/daily-contributions" className={`sidebar-sub-item ${location.pathname === '/daily-contributions' ? 'active' : ''}`} onClick={handleNavigation}>
                                    Daily Contributions
                                </Link>
                                ...
                                <Link to="/leases" className={`sidebar-sub-item ${location.pathname === '/leases' ? 'active' : ''}`} onClick={handleNavigation}>
                                    Property Leases
                                </Link>
                            </div>
                        </div>
                ...
                {/* 4. OPERATIONS */}
                {(isAdmin || roles.includes("COMMUNICATION") || roles.includes("DOCUMENTS") || roles.includes("FORMS") || roles.includes("INVENTORY")) && (
                    <>
                        <div className="sidebar-menu-category" style={{marginTop: '16px'}}>OPERATIONS</div>
                        
                        {/* Communication */}
                        {(isAdmin || roles.includes("COMMUNICATION")) && (
                            <div className={`sidebar-collapsible ${openMenus.communication ? 'is-open' : ''}`}>
                        ...
                        {/* Documents */}
                        {(isAdmin || roles.includes("DOCUMENTS")) && (
                            <Link to="/documents" className={`sidebar-nav-item ${location.pathname === '/documents' ? 'active' : ''}`} onClick={handleNavigation}>
                                <Icon path={<><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></>} />
                                <span>Documents</span>
                            </Link>
                        )}
                        ...
                        {/* Assets */}
                        {(isAdmin || roles.includes("INVENTORY")) && (
                            <Link to="/assets" className={`sidebar-nav-item ${location.pathname === '/assets' ? 'active' : ''}`} onClick={handleNavigation}>
                                <Icon path={<><rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></>} />
                                <span>Assets</span>
                            </Link>
                        )}
                    </>
